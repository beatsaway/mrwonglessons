(function () {
  ...
  function buildVocabEx(vocabObj) {
  ...
  function renderContent(sets, sessionIndex) {
    ...
    if (set.vocabex) {
      html += `<span class="vocab-link" data-exercise="${sessionIndex}">Fill in blanks</span>`;
    }
    html += `<a href="${wikiUrlFor(currentTopic)}" target="_blank" rel="noopener">Wikipedia</a>`;
    html += `</div>`;

    if (set.vocablist && set.vocablist.length) {
      html += `<div class="vocab-list"><div class="vocab-items">`;
      if (set.vocabex) {
        html += `<span class="vocab-list-title" data-exercise="${sessionIndex}">Fill in the blanks with</span>`;
      } else {
    ...
    if (set.articleText) {
      html += `<div class="article-text" id="${setId}">${set.articleText}</div>`;
    }
